function [alignedPinhole2_hv,difference,targetEdge]=verticallyAlignPinholes_2D_v2(inputPinhole1,inputPinhole2,averageRange,vertMax,horzMax,stuck,manualShift)
...
%to Pinhole 1. 
    ...
    warning('off') %I turn off the warning for the fit functions because they flood the command window.
        ...
        lineout1=mean(inputPinhole1(:,(horzMax-averageRange):(horzMax+averageRange)),2,"omitnan"); %Generating lineout through pinhole 1
        lineout2=mean(inputPinhole2(:,(horzMax-averageRange):(horzMax+averageRange)),2,"omitnan"); %Generating lineout through pinhole 2
        ...
        regionToFit1=lineoutToFit1(1:vertMax+50); %Isolating the target edge region to fit
        regionToFit2=lineoutToFit2(1:vertMax+50); %Isolating the target edge region to fit
        ...
        % sing flip suffices.
        targetEdge=length(lineout1)-alignmentPoint1;
...
% 
%     figure()
%     plot((1/max(lineout1))*(lineout1),'b-','LineWidth',2);
%     hold on
%     plot((1/max(lineout2))*((lineout2)),'r-','LineWidth',2);
...
%     figure()
%     plot((1/max(lineout1))*(lineout1),'b-','LineWidth',1.5)
%     hold on
%     %plot(normalize(lineout2),'r--','LineWidth',1)
%     %hold on
%     plot((1/max(nanspace))*(nanspace(range-difference)),'r-','LineWidth',1.5)
