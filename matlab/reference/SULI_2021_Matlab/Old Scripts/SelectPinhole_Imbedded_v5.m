function [Final,max_x,max_y,Final_b,diff_y]=SelectPinhole_Imbedded_v5(RawData,params,col,row,max_col,max_row,smooth_type,smooth_pixels)
...
% 
% RawData=smoothdata(RawData,smooth_type,pixels);
% T=transpose(RawData);
...
Z=Guess(300:800,200:800); %Specifying yet another guess region
...
Z=filloutliers(Z,'linear','mean',31);
...
Z=Final(300:800,200:800); %Specifying yet another guess region

[old_y_max, old_x_max] = find(ismember(Z, max(Z(:)))); %Finds the coordinates of the max in the region 
%(i.e., this is another guess we have to use in order to avoid non-localized maxmia)

max_y=old_y_max+299; %Taking the y-coordiantes of the maxima in the 2nd guess region and transform them into units of the 1st guess region
