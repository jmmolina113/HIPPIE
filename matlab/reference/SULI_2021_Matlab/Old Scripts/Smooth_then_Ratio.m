...
[Aligned1,Aligned2]=AllignPinholes_v2(Final,Final2,Avg_Range(i),max_x,max_y,max_x2,max_y2);
...
norm=max(A);
A=(1/norm)*A; %Step 6
A=smoothdata(A,'movmean',pixel_range);
lineout(:,i)=A;
% 
% plot(Z,lineout(:,i))
% hold on
...
ylabel('Difference in Median Signal Ratio')

N=abs(mean(S_y)-mean(Signal_Ratio));

figure()
plot(2*Avg_Range,N)
xlabel('Range we average over')
ylabel('Difference in Mean Signal Ratio')

O=abs(360-Electron_Temp);

figure()
plot(2*Avg_Range,O)
xlabel('Range we average over')
ylabel('Difference in Electron Temperature')
