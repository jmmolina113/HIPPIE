function [Te,Final,Final2]=EstimateElectronTemperature(shot_location,cal_location,shot_seq_port,where_to_look,row,col1,FilterFileName,FilterFileName2,t1,t2,one_or_two)
...
col2=col1+1;
[lineout,lineout2,S,Final,Final2,R,G]=PinholeAnalysis_WIP(shot_location,cal_location,shot_seq_port,row,col1,col2,smoothing_type,smooth_pixels,where_to_look,Avg_Range,FilterFileName,FilterFileName2,t1,t2);
%S=1./S;
...
% 
%  figure() %%%%%%%%%%%%%%%%%%%%%
% plot(Z,lineout,'b','LineWidth',1.5)
% hold on
% plot(Z,lineout2,'r','LineWidth',1.5)
% set(gca, 'YScale', 'log')
% ylim([0.0001 10])
% xlabel('Z(mm)')
% ylabel('Signal (Arb.)')
...
% % 
% figure()%%%%%%%%%%%%%%%%%%%%%%%%
% plot(Z,S(:,2),'r','LineWidth',1.5)
% hold on
% plot(Z,mean(S(:,2))*((zeros(size(Z,2),1))+1),'--b','LineWidth',1.5)
% ylim([0 0.22])
% xlabel('Z(mm)')
% ylabel('Signal Ratio')
% set(gca,'FontSize',35)
...
% curve2 = (R(:,2))';
% curve3 = (G(:,2))';
% plot(T, curve2, 'w', 'LineWidth', 2);
% x1 = [T, fliplr(T)];
% inBetween = [curve1, fliplr(curve3)];
% fill(x1, inBetween,[0.5 0.5 0.5]);
% hold on
% plot(T, curve2, 'k', 'LineWidth', 2);
% hold on
% % plot(T_S_O,S_O,'sr','MarkerSize',8)
% % hold on
% % plot(T_F_O,F_O,'sg','MarkerSize',8)
% hold on
% plot(T,mean(S(:,2))*(zeros(size(T,2),1)+1),'--r','LineWidth',1.5)
% %hold on
% %xline(TeN,'--r','LineWidth',1.5)
% hold on
% errorbar(TeN,mean(S(:,2)),final_lineout_error,final_lineout_error,T_Error_Low,T_Error_High,'s','MarkerSize',7,'MarkerEdgeColor','blue','MarkerFaceColor','blue','Color','b','LineWidth',1.5)
% xlim([100 1600])
% set(gca, 'YScale', 'log')
% set(gca, 'XScale', 'log')
% eval(['legend("","","Mean Signal Ratio ~ 0.11509","T_{e} ~ ' num2str(round(TeN)) ' eV")'])
% min_R=min(R(:,2));
% ylim([min_R 5])
% ylabel('Ratio')
% xlabel('T_{e} (eV)')
...
% figure()
% 
% plot(T, R(:,2), 'b', 'LineWidth', 2);
% hold on
% plot(T,mean(S(:,2))*(zeros(size(T,2),1)+1),'--r','LineWidth',2)
% hold on
% xline(275.4229,'--g','LineWidth',2)
% xlim([100 1600])
% set(gca, 'YScale', 'log')
% set(gca, 'XScale', 'log')
...
% set(gca,'FontSize',35)
% set(gca,'LineWidth',2)
% legend("R(T_{e})","Mean Signal Ratio ~ 0.11509","T_{e} ~ 275 eV")
% title('Calculated Ratio of X-Ray Photons [R(T_{e})]')
% 
% 
