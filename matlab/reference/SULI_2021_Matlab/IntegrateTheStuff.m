function [Integral]=IntegrateTheStuff(Energies,Temps,Trans_Coeffs)
...
Integral=zeros(size(Temps,2),1);
    ...
    for i=1:size(Temps,2)
        ...
        P=B.*expo.*W.*K; %Product to integrate over
        Integral(i)=trapz(Energies,P); %Integrate the Product
    end
